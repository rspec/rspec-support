  ...
  module Support
    describe Differ do
      describe '#diff' do
        ...
        let(:doc_1) { Nokogiri::HTML(html) }
        let(:doc_2) { Nokogiri::HTML(html) }

        it "outputs unified diff when string" do
          diff = differ.diff(doc_1.to_s,doc_2.to_s)
          expect(diff).to be_diffed_as("\n")
        end

        it "outputs unified diff when inspect string" do
          diff = differ.diff(doc_1.inspect,doc_2.inspect)
          expect(diff).to be_diffed_as('')
        end

        it "handles fairly big diffs just slowly" do
          str = ''
          1000000.times { str << "aasdfghgfdsasdgfd\n" }
          diff = differ.diff(str,str)
          expect(diff).to be_diffed_as("\n")
        end

        it "cant handle 2 nokogiri documents" do
          Timeout.timeout(30) do
            diff = differ.diff(doc_1,doc_2)
            expect(diff).to be_diffed_as('')
          end
        end
      end
